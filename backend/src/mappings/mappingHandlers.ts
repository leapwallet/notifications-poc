import { TransferEvent, Message, Transaction, Transfer } from "../types";
import {
  CosmosEvent,
  CosmosBlock,
  CosmosMessage,
...
export async function handleTransfer({tx, idx, msg}: CosmosMessage): Promise<void> {
  await Transfer
    .create({
      id: `${tx.hash}-${idx}`,
      fromAddress: msg.decodedMsg.fromAddress,
      toAddress: msg.decodedMsg.toAddress,
      denom: msg.decodedMsg.amount[0].denom,
      quantity: msg.decodedMsg.amount[0].amount,
    })
    .save();
}
